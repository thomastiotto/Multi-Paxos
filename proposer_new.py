...
class Proposer:
	...
	def handle_catchupreq(self, msg_catchupreq):
		...
		if self.is_leader():
			...
			self.state[msg_catchupreq.instance_num] = hp.Instance(msg_catchupreq.instance_num, self.id, None)

			logging.debug("Proposer {} \n\tReceived message CATCHUPREQ from Learner {}".format(self.id,
			                                                                                   msg_catchupreq.sender_id))

			msg_1a = hp.Message.create_1a(msg_catchupreq.instance_num, self.id,
			                              self.state[msg_catchupreq.instance_num].c_rnd + hp.NUM_PROPOSERS)
	...
	def handle_2b(self,
	              msg_2b):  # TODO anche se non è leader deve registrarsi l'ultima istanza per rimanere sincronizzato
